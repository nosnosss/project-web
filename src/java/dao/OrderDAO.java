...
public class OrderDAO {
    ...
    public boolean cancelOrder(int orderId) throws SQLException {
        String query = "UPDATE Orders SET status_order = 'canceled' WHERE id = ? AND status_order = 'pending'";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
    ...
    public boolean reorder(int orderId) throws SQLException {
        String query = "UPDATE Orders SET status_order = 'pending' WHERE id = ? AND status_order = 'canceled'";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, orderId);
            int rowsAffected = stmt.executeUpdate();
